...
import { checkLoanMatch, fundLoan, getContract, getLenderOffer, getLoanRequest } from "../lib/contract1";
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-emerald-300">Browse, match, and fund confidential loans</p>
            </div>
          </div>
          <button
            onClick={loadMarketplaceData}
            disabled={loading}
            className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-700 disabled:cursor-not-allowed rounded-lg text-white text-sm font-medium transition"
          >
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-indigo-300/80 mt-1">
              Credit scores, income, and liabilities remain encrypted using FHEVM. Lenders never see sensitive borrower
              data. Matching happens through encrypted comparisons on-chain.
            </p>
          </div>
        </div>
      </div>
      ...
      {matchedPairs.length > 0 && (
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-2xl p-6 shadow-2xl">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="px-3 py-1 bg-green-600/20 border border-green-500/30 rounded-full text-green-400 text-xs font-medium">
          ...
          <div className="space-y-4">
            {matchedPairs.map(match => (
              <div
                ...
                className="bg-slate-900/50 border border-green-600/30 rounded-xl p-6 hover:border-green-500/50 transition"
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <button
                      ...
                      className="px-4 py-2 bg-green-600 hover:bg-green-700 disabled:bg-slate-600 disabled:cursor-not-allowed rounded-lg text-white text-sm font-medium transition whitespace-nowrap"
                    >
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 space-y-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="px-2 py-1 bg-blue-600/20 border border-blue-500/30 rounded text-xs text-blue-400">
                          Active
                        </span>
                        {existingMatch && (
                          <span className="px-2 py-1 bg-green-600/20 border border-green-500/30 rounded text-xs text-green-400">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-white">{new Date(loan.timestamp * 1000).toLocaleDateString()}</p>
                        </div>
                      </div>
                      ...
                      {activeOffers.length > 0 ? (
                        <>
                          ...
                          <button
                            ...
                            disabled={matching || selectedOfferId === undefined || selectedOfferId < 0 || !canMatch}
                            className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600 disabled:cursor-not-allowed rounded-lg text-white text-sm font-medium transition"
                          >
                            {matching ? "Checking..." : "Check Match"}
                          </button>
                          {!canMatch && selectedOfferId !== undefined && selectedOfferId >= 0 && (
                            <p className="text-xs text-amber-400 text-center">Only borrower or lender can match</p>
                          )}
                        </>
                      ) : (
