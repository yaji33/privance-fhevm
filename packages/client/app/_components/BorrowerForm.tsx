...
import { encryptData, getContract, hasCreditScore, hasSubmittedData } from "../lib/contract1";
...
export default function BorrowerForm() {
  ...
  // Check on-chain state when address changes
  useEffect(() => {
    checkOnchainState();
  }, [address]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Clear form only if this was a new submission
      if (!hasData) {
        setForm({
          income: "",
          repaymentScore: "",
  ...
  return (
    <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-2xl p-8 shadow-2xl">
      ...
      {/* Info Box - Only show in initial state */}
      {formState === "initial" && (
